...
void u_green_init(struct uwsgi_server *uwsgi) {
	...
	for(i=0;i<uwsgi->async;i++) {
		...
		uwsgi->ugreen_contexts[i]->uc_stack.ss_sp = mmap(NULL, GREEN_STACK_SIZE + uwsgi->page_size*2 , PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANON | MAP_PRIVATE, -1, 0) + uwsgi->page_size;
