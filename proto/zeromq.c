...
static uint16_t uwsgi_mongrel2_json_add(struct wsgi_request *wsgi_req, json_t *node, const char *json_key, char *key, uint16_t keylen, char **extra, size_t *extra_len) {
	...
	json_t *json_value = json_object_get(node, json_key);
		...
		// invalid value ?
		if (strlen(json_val) > 0xffff) return 0;
		if (extra) {
			*extra = json_val;
...
static int uwsgi_mongrel2_json_parse(json_t *root, struct wsgi_request *wsgi_req) {
	...
	char *json_val;
	char *query_string = NULL; size_t query_string_len = 0;
...
static int uwsgi_mongrel2_tnetstring_parse(struct wsgi_request *wsgi_req, char *buf, int len) {
	...
	char *key = NULL; size_t keylen = 0;
	char *val = NULL; size_t vallen = 0;
...
int uwsgi_proto_zeromq_accept(struct wsgi_request *wsgi_req, int fd) {
	...
	zmq_msg_t message;
	char *req_uuid = NULL; size_t req_uuid_len = 0;
	char *req_id = NULL; size_t req_id_len = 0;
	char *req_path = NULL; size_t req_path_len = 0;
#ifdef UWSGI_JSON
	json_t *root;
        json_error_t error;
#endif
	char *mongrel2_req = NULL; size_t mongrel2_req_size = 0;
	int resp_id_len;
	uint32_t events = 0;
