...
PS_WRITE_FUNC(uwsgi) {
	...
	int vallen = 0;
	if (!uwsgi_cache_magic_set((char *)key, strlen((char *)key), (char *)val, vallen, 0, UWSGI_CACHE_FLAG_UPDATE, cache)) {
		return SUCCESS;	
	}
	return FAILURE;
}
...
PS_DESTROY_FUNC(uwsgi) {
	char *cache = PS_GET_MOD_DATA();
	if (!uwsgi_cache_magic_del((char *)key, strlen((char *)key), cache)) {
		return SUCCESS;
	}
	return FAILURE;
}
